...
export type DirectionCheckResult = {
  advisorFired: boolean;
  directions: DirectionOption[];
};

export async function checkDirectionAdvisor(
  projectId: string,
  message: string,
  ...
  try {
    ...
    return { advisorFired: false, directions: [] };
  }
}

export async function postDirectionPick(
  projectId: string,
  philosophyId: string,
): Promise<void> {
  try {
    ...
    // Best-effort — run proceeds even if recording fails.
  }
}
