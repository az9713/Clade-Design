...
import { createProcessStampArgs, stopProcesses, waitForProcessExit } from "@clade/platform";
...
export async function startPackagedSidecars(
  ...
  try {
    const daemon = await spawnSidecarChild({
      app: APP_KEYS.DAEMON,
      entryPath: resolveSidecarEntry("@clade/daemon", "sidecar"),
      env: {
    ...
    const web = await spawnSidecarChild({
      app: APP_KEYS.WEB,
      entryPath: resolveSidecarEntry("@clade/web", "sidecar"),
      env: {
