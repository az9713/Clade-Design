...
export interface ComposeInput {
  ...
  directionPhilosophy?: string | undefined;
  ...
  if (craftBody && craftBody.trim().length > 0) {
    const sectionLabel =
      Array.isArray(craftSections) && craftSections.length > 0
        ...
        : '';
    parts.push(
      `\n\n## Active craft references${sectionLabel}\n\nThe following craft rules are universal — they apply on top of the active brand context above, regardless of brand. The brand context decides *which* tokens to use; craft rules decide *how* to use them. On any conflict between a craft rule and a brand token, the brand wins for token values; craft rules still apply to anything the brand does not override (letter-spacing, accent overuse caps, anti-slop patterns).\n\n${craftBody.trim()}`,
    );
  }

  // Layer 6: direction philosophy (populated by Phase 4 direction advisor).
  if (directionPhilosophy && directionPhilosophy.trim().length > 0) {
    parts.push(
      `\n\n## Direction philosophy\n\n${directionPhilosophy.trim()}`,
    );
  }
