...
describe('composeSystemPrompt — cladeBrainSnapshot', () => {
  ...
  it('omits brand context block when snapshot is absent', () => {
    const out = composeSystemPrompt({});
  ...
  it('brand snapshot takes precedence over designSystemBody', () => {
    ...
    expect(out).toContain('## Active brand context');
    expect(out).toContain('accent: #FF6B00');
    expect(out).not.toContain('## Active design system');
    expect(out).not.toContain('Legacy DS');
  });

  it('falls back to designSystemBody when snapshot is absent', () => {
    const out = composeSystemPrompt({
      designSystemBody: '# Static DS\n\n## colors\n  primary: #123456\n',
      designSystemTitle: 'Legacy DS',
    });
    expect(out).toContain('## Active design system — Legacy DS');
    expect(out).not.toContain('## Active brand context');
  });

  it('injects directionPhilosophy when present', () => {
    const out = composeSystemPrompt({
      directionPhilosophy: 'Use bold, high-contrast colour language.',
    });
    expect(out).toContain('## Direction philosophy');
    expect(out).toContain('bold, high-contrast');
  });

  it('omits direction philosophy block when absent', () => {
    const out = composeSystemPrompt({});
    expect(out).not.toContain('## Direction philosophy');
  });
});
